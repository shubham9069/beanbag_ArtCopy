...
// first crousel 
  ...
  const Add_to_cart= async(id) =>{
    ...
    const HandlePropagationError=(e,product_id)=>{
      ...
      return (
        <>
        <h2 className="section-heading" Style={'color: #333;font-weight:400 !important'} > <span className="section-heading" style={{color:"#ec1312d1"}}>{title[0]}</span> {title[1]}</h2>
       ...
       <div onClick={()=>navigate('/ProductDetails/' + element.product_id)} className="sliderbg2" >
        ...
        <h4 style={{fontSize:'18px',fontWeight:400}}>{element.name}</h4>
              ...
              <div className='d-flex ' style={{flexWrap:'wrap',gridGap:'10px'}}>
                <button className="themeButton" onClick={(e)=>HandlePropagationError(e,element?.product_id)} style={{fontSize:'10px'}}>Add To Cart</button>
