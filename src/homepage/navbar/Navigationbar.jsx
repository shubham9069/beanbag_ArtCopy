...
const Navigationbar = () => {
  ...
  const {userToken,userData,Cart,All_Product_Page,Catagory,CategoryData} =useContext(AuthContext);
...
useEffect(() => {
  ...
  return (
<>
<div className='between-div' style={{height:'100%',width:'100%', background: '#f5f6f8',gridGap:10,color:'white',flexWrap:'wrap',padding:'0.8rem'}}>
     <a style={{color:'#333',fontSize:'12px'}}>WELCOME TO BEANBAG STORE MESSAGE OR REMOVE IT!</a>
     <a  style={{color:'#333',fontSize:'12px'}}><i class="bi bi-telephone" Style={'font-size:12px !important'}></i> 91 9069232229</a>
    ...
    <span class="navbar-toggler-icon"></span>
      ...
      <div className="searchbaron1000 d-flex " style={{margin:'0 auto',position:'relative',    width: '100%', flex: 1}}>
 
 <div style={{maxWidth:'450px',width:'100%',position:'relative', display:'flex',margin:'0 auto'}} onClick={()=>setSearchShow(!searchShow)}>
 <input type="text" className="form-control" id="search" placeholder="Search bar "
  ...
  {/* <div className="d-flex " style={{gridGap:10}}>
        ...
        <ul className="navbar-nav " style={{gridGap:'10px',margin:'0 0 0 auto'}}>
          <li className="nav-item">
            <Link to="/" className="nav-link nav-btn ">abouts</Link>
          </li>
          ...
          <li className="nav-item">
            <Link to='/AllArtist' className="nav-link nav-btn">blogs</Link>
          </li>
          <li className="nav-item" >
  ...
  <div className='d-flex Buttonofnavbar dropdown  '>
          ...
          (e.target.onerror = null)(
            (e.target.src =
              "images/userdefault.png")
                    ...
                    ></img>  <span style={{color:'black'}}>{userData?.name}</span>
                  </Dropdown.Toggle>
    ...
    <div className='center-div' style={{height:'100%',width:'100%',background:'black',gridGap:10,color:'white',flexWrap:'wrap',padding:'0.8rem'}}>
     {CategoryData?.map((element)=>{

      return <Link className='link-a px-2' >{element?.name}</Link>
     })}
    </div>
